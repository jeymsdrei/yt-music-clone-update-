...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-white/60 mt-1">Add a new album to the platform</p>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('admin.albums.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="release_year" value="{{ old('release_year', date('Y')) }}" min="1900" max="{{ date('Y') + 1 }}" class="input-field @error('release_year') border-red-500 @enderror">
                    @error('release_year')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
