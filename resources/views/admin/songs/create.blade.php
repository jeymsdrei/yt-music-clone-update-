...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold">Upload New Song</h2>
            <p class="text-white/60 mt-1">Add a new song to the platform</p>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('admin.songs.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" class="input-field @error('title') border-red-500 @enderror" required>
                    @error('title')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="genre" value="{{ old('genre') }}" placeholder="e.g. Rock, Pop, Hip-Hop" class="input-field @error('genre') border-red-500 @enderror">
                    @error('genre')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="duration" value="{{ old('duration') }}" min="1" class="input-field @error('duration') border-red-500 @enderror" required>
                    @error('duration')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                ...
                <div class="relative border-2 border-dashed border-white/10 rounded-xl p-8 text-center hover:border-red-500/50 transition-colors cursor-pointer" onclick="document.getElementById('audio-file').click()">
                    <input type="file" id="audio-file" name="file" accept=".mp3,.wav,.ogg,.flac" class="hidden" onchange="updateFileInfo(this, 'file-info')" required>
            ...
            <div>
                ...
                <div class="relative border-2 border-dashed border-white/10 rounded-xl p-6 text-center hover:border-red-500/50 transition-colors cursor-pointer" onclick="document.getElementById('cover-image').click()">
                    ...
                    <p class="text-sm text-white/60">Upload cover image</p>
                </div>
            ...
            <div>
                ...
                <textarea name="lyrics" rows="6" class="input-field @error('lyrics') border-red-500 @enderror">{{ old('lyrics') }}</textarea>
                @error('lyrics')
                <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center gap-3">
                <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured') ? 'checked' : '' }} class="w-4 h-4 rounded border-white/20 bg-surface text-red-500 focus:ring-red-500">
