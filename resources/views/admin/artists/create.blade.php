...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold">Create Artist</h2>
            <p class="text-white/60 mt-1">Add a new artist to the platform</p>
        </div>
    </div>
    ...
    <div class="glass-card rounded-xl p-6">
        <form method="POST" action="{{ route('admin.artists.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="bio" rows="5" class="input-field @error('bio') border-red-500 @enderror">{{ old('bio') }}</textarea>
                @error('bio')
                <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="relative border-2 border-dashed border-white/10 rounded-xl p-6 text-center hover:border-red-500/50 transition-colors cursor-pointer" onclick="document.getElementById('image').click()">
                    ...
                    <p class="text-sm text-white/60">Upload profile image</p>
                </div>
            ...
            <div class="flex items-center gap-3">
                <input type="checkbox" name="verified" id="verified" value="1" {{ old('verified') ? 'checked' : '' }} class="w-4 h-4 rounded border-white/20 bg-surface text-red-500 focus:ring-red-500">
                <label for="verified" class="text-sm text-white/80">Verified artist</label>
            </div>

            <div class="flex items-center gap-3 pt-4 border-t border-white/10">
                <button type="submit" class="btn-primary">Create Artist</button>
